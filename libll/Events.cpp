...
//NM Needed for CLUSTER_ARCH
void rsesc_initialize_reconfiguration(int numRegions)
{
  osSim->initializeReconfiguration(numRegions);
}

int rsesc_begin_region(int numRegion)
{
  return osSim->beginRegion(numRegion);
}

void rsesc_end_region(int numRegion)
{
  osSim->endRegion(numRegion);
}

void rsesc_start_next_application(char *appName)
{
#ifdef MULTI_WORKLOAD
  osSim->startNextApplication(appName); 
#endif
}
