...
#define k_interleavedCache "interleavedCache"
//NM Rambus (memCntrl + Bank) added by NM
#define k_RambusBank     "RambusBank"
#define k_RambusMemCntrl "RambusMemCtrl"
...
{ 
  ...
  } else if(!strcasecmp(device_type,k_interleavedCache)) {
   
    new_memory_device = new IntrlvdCache(this, device_descr_section, device_name);

  } else if (!strcasecmp(device_type, k_RambusBank)) { // nice cache always hits

    new_memory_device = new RambusBank(this, device_descr_section, device_name);

  } else if (!strcasecmp(device_type, k_RambusMemCntrl)) {

    new_memory_device = new RambusMemCtrl(this, device_descr_section, device_name);
