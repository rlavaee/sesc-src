...
//***********CLUSTER_ARCH
  {"sesc_begin_region",       mint_begin_region,               1, OpExposed},
  {"sesc_end_region",         mint_end_region,                 1, OpExposed},
  {"sesc_set_num_region",     mint_set_num_region,             1, OpExposed},
  {"sesc_start_next_application", mint_start_next_application, 1, OpExposed},
//****************
...
void subst_init()
{
  ...
  char msg0[] = "__errnoaddr";
  errno_nlist[0].n_name = msg0;
  char msg1[] = "sesc_spawn";
  errno_nlist[1].n_name = msg1;
  char msg2[] = "exit";
  errno_nlist[2].n_name = msg2;
  char msg3[] = "environ";
  errno_nlist[3].n_name = msg3;   
  char msg4[] = "m_fork";
  errno_nlist[4].n_name = msg4;   
  char msg5[] = "sprocsp";
  errno_nlist[5].n_name = msg5;   
  ...
  if (errno_nlist[2].n_type == 0 || errno_nlist[2].n_value == 0){
    char msg[] = "subst_init: cannot find address of exit()\n"; 
    fatal(msg);
  }

  pnlist = (struct namelist *) malloc(sizeof(struct namelist) * Nfuncs);
  if (pnlist == NULL){
    char msg[] = "subst_init: cannot allocate enough bytes for function nlist.\n"; 
    fatal(msg);
  }
  for (i = 0; i < Nfuncs; i++){
...
{
  char msg[128];
  sprintf(msg,"System call @0x%lx not implemented. Use the appropiate API\n",
	  picode->addr); 
  fatal(msg);
...
//needed for CLUSTER_ARCH
OP(mint_begin_region){
  ID(Pid_t thePid = pthread->getPid());
  // Arguments of the sesc
  int region = pthread->getGPR(Arg1stGPR);

  int numThreads = rsesc_begin_region(region);

  pthread->setGPR(RetValGPR,numThreads);
...
OP(mint_end_region){
  ...
  rsesc_end_region(region);

  I(pthread->getPid() == thePid);
  return addr2icode(pthread->getGPR(RetAddrGPR));
}

OP(mint_set_num_region){
  ID(Pid_t thePid = pthread->getPid());
  // Arguments of the sesc
  int numRegions = pthread->getGPR(Arg1stGPR);

  rsesc_initialize_reconfiguration(numRegions);

  I(pthread->getPid() == thePid);
  return addr2icode(pthread->getGPR(RetAddrGPR));
}

OP(mint_start_next_application){
  ID(Pid_t thePid = pthread->getPid());
  // Arguments of the sesc
  char * appName = (char *) (pthread->virt2real(pthread->getGPR(Arg1stGPR)));
  printf("MINT:sesc_start_next_application    appName = %s\n", appName);
  rsesc_start_next_application(appName);
...
OP(mint_sesc_spawn){
  ...
  if(!child){
    char msg[128];
    sprintf(msg,"sesc_spawn: exceeded process limit (%d)\n", Max_nprocs); 
    fatal(msg);
  }
...
int mint_sesc_create_clone(ThreadContext *pthread){
  ...
  if(cthread==NULL) {
    char msg[128];
    sprintf(msg, "mint_sesc_create_clone: exceeded process limit (%d)\n",Max_nprocs); 
    fatal(msg);
  }
  cpid=cthread->getPid();
  if(Maxpid<cpid)
...
OP(mint_sysmp)
{
    ...
    switch (command) {
      ...
      default:
	char msg[128];
	sprintf(msg,"mint_sysmp( 0x%x ) not supported yet.\n", (unsigned) command); 
        fatal(msg);
        break;
    }
    return addr2icode(REGNUM(31));
...
OP(mint_printf)
{
    ...
    if(index>10){
      char msg[128];
      sprintf(msg,"mint_printf: too many args\n"); 
      fatal(msg);
    }
#endif
...
OP(mint_ioctl)
{
  ...
  default:
    char msg[128];
    sprintf(msg,"ioctl command %d (0x%x) not supported.\n", (unsigned)cmd, (unsigned)cmd); 
    fatal(msg);
    break;
  }
...
OP(mint_prctl)
{
	...
	/* not really supported, but just fake it for now */
      ...
      default:
	char msg[128];
	sprintf(msg,"prctl option %ld not supported yet.\n", option);
        fatal(msg);
        break;
    }
    REGNUM(2) = err;
...
OP(mint_fcntl)
{
  ...
  case F_SETLKW:
    {
      char msg[128];
      sprintf(msg,"mint_fcntl: system call fcntl cmd `F_SETLKW' not supported.\n");
      fatal(msg);
    }
    break;
  case F_GETOWN:
  ...
  default:
    {
      char msg[128];
      sprintf(msg,"mint_fcntl: system call fcntl cmd (%ld) not expected.\n", cmd);
      fatal(msg);
    }
  }
...
#endif
  if(r4 == 0) {
    char msg[] = "uname called with a null pointer\n"; 
    fatal(msg);
  }
...
OP(mint_execl)
{
    ...
    printf("mint_execl()\n");
#endif
    char msg[] = "execl() system call not yet implemented\n"; 
    fatal(msg);
    return NULL;
}
...
OP(mint_execle)
{
    ...
    printf("mint_execle()\n");
#endif
    char msg[] = "execle() system call not yet implemented\n"; 
    fatal(msg);
    return NULL;
}
...
OP(mint_execlp)
{
    ...
    printf("mint_execlp()\n");
#endif
    char msg[] = "execlp() system call not yet implemented\n"; 
    fatal(msg);
    return NULL;
}
...
OP(mint_execv)
{
    ...
    printf("mint_execv()\n");
#endif
    char msg[] = "execv() system call not yet implemented\n"; 
    fatal(msg);
    return NULL;
}
...
OP(mint_execve)
{
    ...
    printf("mint_execve()\n");
#endif
    char msg[] = "execve() system call not yet implemented\n"; 
    fatal(msg);
    return NULL;
}
...
OP(mint_execvp)
{
    ...
    printf("mint_execvp()\n");
#endif
    char msg[] = "execvp() system call not yet implemented\n"; 
    fatal(msg);
    return NULL;
}
...
OP(mint_mmap)
{
  ...
  if( r4 ) {
    ...
    char msg[] = "mmap called with start different than zero\n"; 
    fatal(msg);
  }
...
#endif
  if(r4 == 0 || r5 == 0) {
    char msg[] = "stat called with a null pointer\n"; 
    fatal(msg);
  }
...
#endif
  if(r5 == 0) {
    char msg[] = "fstat called with a null pointer\n"; 
    fatal(msg);
  }
...
#endif
  if(r4 == 0 || r5 == 0) {
    char msg[] = "lstat64 called with a null pointer\n"; 
    fatal(msg);
  }
...
#endif
  if(r4 == 0 || r5 == 0) {
    char msg[] = "stat64 called with a null pointer\n"; 
    fatal(msg);
  }
...
OP(mint_getcwd)
{
  ...
  if(r4 == 0) {
    char msg[] = "getcwd called with a null pointer\n"; 
    fatal(msg);
  }
...
OP(mint_syssgi)
{
  ...
  case SGI_GETNVRAM:
    {
      char msg[128];
      sprintf(msg,"mint_syssgi: command 0x%x not supported yet.\n", (unsigned) r4); 
      fatal(msg);
    }
    break;
  case SGI_RUSAGE:
  ...
  case SGI_INVENT:
	...
	r6 = pthread->virt2real(r6);
      ...
      char msg[128];
      sprintf(msg,"mint_syssgi: SGI_INVENT: arg 0x%x not expected.\n", (unsigned)r5); 
      fatal(msg);
    }
    break;
  ...
  default:
    char msg[128];
    sprintf(msg,"mint_syssgi: command 0x%x not expected.\n", (unsigned) r4);
    fatal(msg);
    break;
  }
  return addr2icode(REGNUM(31));
