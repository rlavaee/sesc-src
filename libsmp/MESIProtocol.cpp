...
void MESIProtocol::write(MemRequest *mreq)
{
  ...
  // hit in shared state
  if (l && !l->canBeWritten()) {
     GLOG(SMPDBG_MSGS, "MESIProt::write (hit in shared) for addr 0x%x, type = %d, @%lld",
  ...
  if(!l) {
    GLOG(SMPDBG_MSGS, "MESIProt::write (miss, couldn't alloc line) for addr 0x%x, type = %d, @%lld",
          (uint) mreq->getPAddr(), (uint) mreq->getMemOperation(), globalClock);
  ...
  GLOG(SMPDBG_MSGS, "MESIProt::write (miss) for addr 0x%x, type = %d, @%lld",
          (uint) mreq->getPAddr(), (uint) mreq->getMemOperation(), globalClock);
...
{
  GLOG(SMPDBG_MSGS, "MESIProt::doWrite for addr 0x%x, type = %d, @%lld",
         (uint) mreq->getPAddr(), (uint) mreq->getMemOperation(), globalClock);
...
void MESIProtocol::writeMissHandler(SMPMemRequest *sreq) 
{
  ...
  if(l && !l->isLocked()) {
    GLOG(SMPDBG_MSGS, "MESI::writeMissHand (valid invalidated) for addr 0x%x, state = %x, @%lld",
       (uint) sreq->getPAddr(), l->getState(), globalClock);

    combineResponses(sreq, (MESIState_t) l->getState());
    //NM Bug!!! Fixing 2 times writeback problem, invalidateLine will already writeback
    if((sreq->getSupplier() == pCache) && sreq->needsWriteDown()) {
      sreq->clearWriteDown();
    }
    pCache->invalidateLine(addr, sendWriteMissAckCB::create(this, sreq));
    return;
  } else {
    GLOG(SMPDBG_MSGS, "MESI::writeMissHand (invalid-ack) for addr 0x%x, @%lld",
       (uint) sreq->getPAddr(), globalClock);
