  ...
  , Id(dms->getId())
{
  ...
  if (mutExclBuffer == NULL)
      // doris: change to read from conf file
      mutExclBuffer = MSHR<PAddr,SMPCache>::create("mutExclBuffer", 
    				  SescConf->getCharPtr(mshrSection, "type"),
    				  SescConf->getLong(mshrSection, "size"),
    				  SescConf->getLong(mshrSection, "bsize"));
  // hardwared MSHR size  
  // Engin: Changed this because 32K entry mshr makes no sense
  //   mutExclBuffer = MSHR<PAddr,SMPCache>::create("mutExclBuffer", 
  // 						 SescConf->getCharPtr(mshrSection, "type"),
  // 						 16,
  // 						 SescConf->getLong(mshrSection, "bsize"));
...
void SMPCache::access(MemRequest *mreq)
{
  ...
  GLOG(SMPDBG_MSGS, "SMPCache::access cache %s for addr 0x%x, type = %d, @%lld",
       getSymbolicName(), (uint) mreq->getPAddr(), (uint) mreq->getMemOperation(), globalClock);
...
{  
  ...
  if (!outsReq->issue(addr)) {
    ...
    GLOG(SMPDBG_MSGS, "SMPCache::read cannot issue  cache %s for addr 0x%x, type = %d, @%lld",
       getSymbolicName(), (uint) mreq->getPAddr(), (uint) mreq->getMemOperation(), globalClock);
    return;
  }

  GLOG(SMPDBG_MSGS, "SMPCache::read cache %s (can issue) for addr 0x%x, type = %d, @%lld",
         getSymbolicName(), (uint) mreq->getPAddr(), (uint) mreq->getMemOperation(), globalClock);
...
void SMPCache::doRead(MemRequest *mreq)
{
  ...
  if (l && l->canBeRead()) {
    GLOG(SMPDBG_MSGS, "SMPCache::doRead cache %s (h) for addr 0x%x, type = %d, state = 0x%08x, @%lld",
         getSymbolicName(), (uint) mreq->getPAddr(), (uint) mreq->getMemOperation(), (uint) l->getState(), globalClock);
  ...
  GLOG(SMPDBG_MSGS, "SMPCache::doRead cache %s (miss) for addr 0x%x, type = %d, @%lld",
         getSymbolicName(), (uint) mreq->getPAddr(), (uint) mreq->getMemOperation(), globalClock);
...
{  
  ...
  if (!outsReq->issue(addr)) {
    ...
    GLOG(SMPDBG_MSGS, "SMPCache::write cache %s (cannot issue) for addr 0x%x, type = %d, @%lld",
         getSymbolicName(), (uint) mreq->getPAddr(), (uint) mreq->getMemOperation(), globalClock);
    return;
  }
  
  GLOG(SMPDBG_MSGS, "SMPCache::write cache %s (can issue) for addr 0x%x, type = %d, @%lld",
         getSymbolicName(), (uint) mreq->getPAddr(), (uint) mreq->getMemOperation(), globalClock);
...
void SMPCache::doWrite(MemRequest *mreq)
{
  ...
  if (l && l->canBeWritten()) {  
    ...
    GLOG(SMPDBG_MSGS, "SMPCache::doWrite cache %s (hit) for addr 0x%x, type = %d, @%lld",
         getSymbolicName(), (uint) mreq->getPAddr(), (uint) mreq->getMemOperation(), globalClock);

    writeHit.inc();

    if(!l->isDirty()) {  //NM added 
      I((l->getState() == MESI_MODIFIED) || (l->getState() == MESI_EXCLUSIVE));
      protocol->makeDirty(l); //NM added, Bug found!
    }
    mreq->goUp(hitDelay); 
  ...
  if (l && l->isLocked()) {
    GLOG(SMPDBG_MSGS, "SMPCache::doWrite cache %s (miss tr) for addr 0x%x, type = %d, @%lld",
         getSymbolicName(), (uint) mreq->getPAddr(), (uint) mreq->getMemOperation(), globalClock);
  ...
  GLOG(SMPDBG_MSGS, "SMPCache::doWrite cache %s (miss) for addr 0x%x, type = %d, @%lld",
         getSymbolicName(), (uint) mreq->getPAddr(), (uint) mreq->getMemOperation(), globalClock);
  ...
  if (!mutExclBuffer->issue(addr)) {
    ...
    GLOG(SMPDBG_MSGS, "SMPCache::doWrite cache %s hold at mtlBuff for addr 0x%x, type = %d, @%lld",
         getSymbolicName(), (uint) mreq->getPAddr(), (uint) mreq->getMemOperation(), globalClock);
    return;
  }
...
{
  GLOG(SMPDBG_MSGS, "SMPCache::sendWrite cache %s for addr 0x%x, type = %d, @%lld",
         getSymbolicName(), (uint) mreq->getPAddr(), (uint) mreq->getMemOperation(), globalClock);
  protocol->write(mreq);
}
...
{
  GLOG(SMPDBG_MSGS, "SMPCache::sendBelow cache %s for addr 0x%x, type = %d, @%lld",
         getSymbolicName(), (uint) sreq->getPAddr(), (uint) sreq->getMemOperation(), globalClock);
  if(sreq->needsSnoop())
...
void SMPCache::concludeAccess(MemRequest *mreq)
{
  ...
  GLOG(SMPDBG_MSGS, "SMPCache::concludeAccess cache %s for addr 0x%x, type = %d, @%lld",
       getSymbolicName(), (uint) mreq->getPAddr(), (uint) mreq->getMemOperation(), globalClock);
...
void SMPCache::allocateLineRetry(PAddr addr, CallbackBase *cb)
{
...
SMPCache::Line *SMPCache::allocateLine(PAddr addr, CallbackBase *cb, bool firstTry) //NM added firstTry
{
  PAddr rpl_addr = 0;
  I(cache->findLineDebug(addr) == 0);
  Line *l = cache->findLine2Replace(addr);

//  I(l); commented, NM bug found
  nextSlot(); // have to do an access to check which line is free

  if(l == 0) {
    allocateLineRetryCB::scheduleAbs(globalClock + 10, this, addr, cb);          
    return 0;
  } 
  ...
  if(!l->isValid()) {
    if(firstTry) cb->destroy();
    l->setTag(cache->calcTag(addr)); // NM changed cache->fillLine(addr);
    return l;
  }
  ...
  if(isHighestLevel()) {
    ...
    if(firstTry) cb->destroy(); 
    //cache->fillLine(addr);          //NM removed, bug found! Fills another line than l 
    protocol->preInvalidate(l);       //NM added, bug found! 
    l->setTag(cache->calcTag(addr));  //NM added, bug found! Should change the tag of the same l line
    return l;
  }
