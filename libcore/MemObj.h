...
protected:
  ...
  void addLowerLevel(MemObj *obj) { 
    I( obj );
    //printf("Add lower level %s to %s\n", obj->getSymbolicName(), symbolicName);
...
public:
  ...
  ulong getUpperLevelSize() const { return upperLevel.size(); }

  virtual bool isCache() const { return false; }

   ulong getNumCachesInUpperLevels() const {
    return nUpperCaches;
  }
