...
/*******************************/
    ...
    ,regionBeginTime(0)
    ,confTimingHist(0)
    ,totalParallelTime("OSSim:totalParallelTime")
    ,nRegionOccur(0)
/*******************************/
...
//NM********************
        ...
        if( argv[i][2] != 0 )
          parallelSection = &argv[i][2];
        else {
          i++;
          parallelSection = argv[i];
        }
      }else if( argv[i][1] == 'n' ) {
        if( argv[i][2] != 0 )
          paramThreads = strtol(&argv[i][2],0,0);
        else {
          i++;
          paramThreads = strtol(argv[i],0,0);
        } 
        printf("OSSIm:process parameters: input flag n set, paramThreads = %ld\n",paramThreads);               
//**********************
      }else{
...
void OSSim::eventSpawn(Pid_t ppid, Pid_t fid, long flags, bool stopped)
{
  ...
  MSG("OSSim::eventSpawn prefered CPU for processId(%d) is %d", fid, nextProcToSched); 
  procId->setCPU(nextProcToSched);
  nextProcToSched = 0;
#endif

  //Engin: change
#ifdef MULTIPROGRAMMED
  procId->setCPU(4);
#endif
...
#ifdef MULTI_WORKLOAD
  if(parallelSection == 0) {
    MSG("Please provide a workload identification section..."); 
    exit(-1);
  }
  SescConf->isLong(parallelSection,"initialCPU");
  nextProcToSched = SescConf->getLong(parallelSection,"initialCPU");
#endif
...
void OSSim::initializeReconfiguration(int numRegions)
{   
  int numParallelRegions;
    
      
  parallelizeRegion = new bool[numRegions];
  confTimingHist    = new GStatsHist* [numRegions];
  nRegionOccur      = new GStatsCntr* [numRegions];
    
  for(int i=0; i<numRegions; i++){
    parallelizeRegion[i] = false;
    confTimingHist[i]    = new GStatsHist("OSSim:region%d_exeTime",i);
    nRegionOccur[i]      = new GStatsCntr("OSSim:region%d_occur",i);
  }   

  if(SescConf->checkCharPtr(parallelSection,"region"))
    numParallelRegions = SescConf->getRecordSize(parallelSection,"region");
  else
    numParallelRegions = 0;


  for(int i = 0; i<numParallelRegions; i++) {
    int region = strtol(SescConf->getCharPtr(parallelSection,"region",i),0,0);
    parallelizeRegion[region] = true;
  }
}

//Beginning of a parallel region
int OSSim::beginRegion(int regionNumber)
{
  regionBeginTime = globalClock;
  nRegionOccur[regionNumber]->inc();

  if(parallelizeRegion[regionNumber] == false) {
       
    return 1; //Don't parallelize, run it sequential
  }

#ifndef NO_RECONFIGURE
 
#ifdef APPL_CHOICE

  return (int) paramThreads;
...
void OSSim::endRegion(int regionNumber)
{
  confTimingHist[regionNumber]->sample(0, globalClock - regionBeginTime);

  if(parallelizeRegion[regionNumber] == false) {
    return;
  }

  totalParallelTime.sample(0,globalClock-regionBeginTime);

}

void OSSim::startNextApplication(char* appName)
{

#ifdef MULTI_WORKLOAD
  SescConf->isLong(parallelSection, appName);
  nextProcToSched  = SescConf->getLong(parallelSection, appName);
  MSG("OSSim::startNewApplication  nextProcToSched = %d", nextProcToSched);
#endif
}
