...
protected:
 ...
 protected:
  Resource(Cluster *cls, PortGeneric *gen, int id);
public:
  virtual ~Resource();
  ...
  void issued(DInst *);  //NM
  typedef CallbackMember1<Resource, DInst *, &Resource::issued> issuedCB;
...
public:
  ...
  FUGeneric(Cluster *cls, PortGeneric *aGen, TimeDelta_t l, GStatsEnergyCG *eb, int id);
...
private:
  const TimeDelta_t lat;
  const TimeDelta_t misCommLat;
  int freeBranches;

protected:
public:
  FUBranch(Cluster *cls, PortGeneric *aGen, TimeDelta_t l, int mb, TimeDelta_t mcl, int id);
