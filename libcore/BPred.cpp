   ...
   ,nMiss("BPred(%d)_%s:nMiss",i,n)
{
  ...
  bpredEnergy = new GStatsEnergy("bpredEnergy", cadena, i, BPredEnergy, 
				 EnergyMgr::get("bpredEnergy",i));
...
void BPBTB::updateOnly(const Instruction *inst, InstID oracleID)
{
  ...
  //NM: Also changed this since Engin changed it below. 
  //ulong key   = calcInstID(inst);
  ulong key = inst->getAddr();
...
BPA21264::BPA21264(int i, const char *section)
  :BPred(i,section,"A21264"),
   btb(i, section),
   //Get the global history siz and ghr mask
   gHistSize(SescConf->getLong(section,"gHistSize")),
   gHistMask((1 << gHistSize) - 1),
   //Get the local history size and mask
   lHistSize(SescConf->getLong(section,"lHistSize")),
   lHistMask((1 << lHistSize) - 1),
   //Get the bht size and the mask for accessing it
   bhtSize(SescConf->getLong(section,"bhtSize")),
   bhtMask(bhtSize - 1),
   //Get the mask for accessing the meta predictor
   mMask((SescConf->getLong(section,"mSize") - 1)),
   //Create the PHTs
   localTable(i,section,1<<lHistSize,SescConf->getLong(section,"lBits")),
   globalTable(i,section,1<<gHistSize,SescConf->getLong(section,"gBits")),
   metaTable(i,section,SescConf->getLong(section,"mSize"),
	     SescConf->getLong(section,"mBits"))
{ 

  // Constraints
  SescConf->isLong(section,    "lHistSize");
  SescConf->isBetween(section, "lBits", 1, 7);

  SescConf->isLong(section,    "gHistSize");
  SescConf->isBetween(section, "gBits", 1, 7);

  SescConf->isLong(section,    "mSize");
  SescConf->isPower2(section,  "mSize");

  SescConf->isLong(section,    "bhtSize");
  SescConf->isPower2(section,  "bhtSize");

  SescConf->isBetween(section, "mBits", 1, 7);

  bht = (HistoryType *) malloc(sizeof(HistoryType) * bhtSize);
...
PredType BPA21264::predict(const Instruction *inst, InstID oracleID, bool doUpdate)
{

  //Increment predictor energy
  bpredEnergy->inc();
  ...
  //Flag showing if this is a taken branch
  bool taken = (inst->calcNextInstID() != oracleID);
  
  //Randomized branch ID generate from branch PC
  HistoryType iID = calcInstID(inst);

  //GAg index
  HistoryType globalIndex = ghr & gHistMask;
  
  //SAg level 1 index
  HistoryType localL1Index = iID & bhtMask;

  //SAg level 2 index
  HistoryType localL2Index = bht[localL1Index] & lHistMask;

  //Meta Index
  HistoryType metaIndex = iID & mMask;
  ...
  if (doUpdate) {
    ...
    ghr = ((ghr << 1) | (taken ? 1 : 0));
    //BHR update
    bht[localL2Index] = (bht[localL2Index] << 1) | (taken ? 1 : 0);
  }
  ...
  //Oracle updates to PHT's with prediction
  if (doUpdate) {
    globalTaken = globalTable.predict(globalIndex, taken);
    localTaken  = localTable.predict(localL2Index, taken);
  }
  //Predict, but do not perform oracle updates
  else{
    globalTaken = globalTable.predict(globalIndex);
    localTaken  = localTable.predict(localL2Index);
  }

  //Do not update meta
  if (!doUpdate) {
    metaOut = metaTable.predict(metaIndex);
  }
  //Oracle update to meta w/ prediction
  else if( globalTaken == taken && localTaken != taken) {
    // global is correct, local incorrect
    metaOut = metaTable.predict(metaIndex, false);
  }else if( globalTaken != taken && localTaken == taken) {
    // global is incorrect, local correct
    metaOut = metaTable.predict(metaIndex, true);
  }else{
    metaOut = metaTable.predict(metaIndex); // do not update meta
  }

  //Final prediction selected by the meta
  bool ptaken = metaOut ? localTaken : globalTaken;

  //Misprediction
  if (taken != ptaken) {
    //Oracle update to BTB
    if (doUpdate){
      btb.updateOnly(inst,oracleID);
    }
    return MissPrediction;
  ...
  //If taken, return btb's prediction. Otherwise, return corret prediction ?
  return ptaken ? btb.predict(inst, oracleID, doUpdate) : CorrectPrediction;
}

void BPA21264::switchIn(Pid_t pid)
{
#ifdef TASKSCALAR
  TaskContext *tc = TaskContext::getTaskContext(pid);
  if (tc==0 || tc->getPid()==-1)
...
void BPA21264::switchOut(Pid_t pid)
{
#ifdef TASKSCALAR
  TaskContext *tc = TaskContext::getTaskContext(pid);
  if (tc==0 || tc->getPid()==-1)
...
BPHybrid::BPHybrid(int i, const char *section)
  :BPred(i,section,"Hybrid"),
   btb(i, section),
   //Get the global history size and ghr mask
   gHistSize(SescConf->getLong(section,"gHistSize")),
   gHistMask((1 << gHistSize) - 1),
   //Get the local history size and mask
   lHistSize(SescConf->getLong(section,"lHistSize")),
...
{ 
  ...
  //printf("bhtMask=%ld\n",bhtMask);
  //printf("mMask=%ld\n",mMask);
  //printf("lBits=%ld\n",SescConf->getLong(section,"lBits"));
  //printf("gBits=%ld\n",SescConf->getLong(section,"gBits"));
  //printf("mBits=%ld\n",SescConf->getLong(section,"mBits"));
  //printf("mSize=%ld\n",SescConf->getLong(section,"mSize"));
  //fflush(stdout);

  ghr = (HistoryType *) malloc(sizeof(HistoryType));
...
PredType BPHybrid::predict(const Instruction *inst, InstID oracleID, bool doUpdate)
{
  ...
  HistoryType iID = inst->getAddr() >> 2;
  ...
  if (doUpdate) {
    ...
    *ghr = ((*ghr << 1) | (taken ? 1 : 0));
    //BHR update
    bht[localL1Index] = (bht[localL1Index] << 1) | (taken ? 1 : 0);
  }
...
BPred *BPredictor::getBPred(int id, const char *sec)
{
  ...
  } else if (strcasecmp(type, "A21264") == 0) {
    pred = new BPA21264(id,sec);
  } else {
