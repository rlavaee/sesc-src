...
private:
  ...
  //Engin:
  //ICache block size for alignment (and its logarithm)
  long iCacheBlockSizeLog2;
  //Last PC remembered for alignment checks
  long lastPC;
  //Number of instructions to be fetched at each call to realFetchAligned
  long n2FetchedAligned;

  GStatsCntr nMispathFetchCycles; //NM
protected:
...
public:
  ...
  //Aligned fetch functions
  bool fetchAligned(IBucket *buffer, bool resume, int fetchMax = -1);
  bool realFetchAligned(IBucket *buffer, bool resume, int fetchMax = -1);
