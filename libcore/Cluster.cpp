...
)
{
  ...
  case iDiv:
   ...
   r = new FUGeneric(cluster, gen, lat, eng, gproc->getId());
   break;
  case fpALU:
  ...
  case iBJ:
	...
	MaxBranches = INT_MAX;
      ...
      TimeDelta_t misCommLat = 0;

      if( SescConf->checkLong("cpucore", "brMisCommLat", gproc->getId() ))
        misCommLat = SescConf->getLong("cpucore", "brMisCommLat", gproc->getId());
      else
        MSG("brMisCommLat not provided in configuration file! Set to default value of 0.");

      r = new FUBranch(cluster, gen, lat, MaxBranches, misCommLat, gproc->getId());
    }
    break;
  case iLoad:
    {
  ...
  case iStore:
	...
	I(res[iEvent]==0);
	res[iEvent] = new FUEvent(cluster, gproc->getId());
      }
    }
    break;
  default:
...
void IssuedCluster::executed(DInst *dinst)
{
  dinst->markExecuted();
  window.executed(dinst);
}

void IssuedCluster::retire(DInst *dinst)
{
...
//************ ClusterManager
static long intWindow=0;
static long fpWindow=0;
