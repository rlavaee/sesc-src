...
int sesc_begin_region(int region){
  fprintf(stderr,"sesc_begin_region shouldn't be called without a simulator. This is a simulator only code\n");
  exit(-1);
}
int sesc_end_region(int region){
  fprintf(stderr,"sesc_end_region shouldn't be called without a simulator. This is a simulator only code\n");
  exit(-1);
}
int sesc_set_num_region(int region){
  fprintf(stderr,"sesc_begin_region shouldn't be called without a simulator. This is a simulator only code\n");
  exit(-1);
}

void sesc_start_next_application(char * name) {
  fprintf(stderr,"sesc_start_next_application shouldn't be called without a simulator. This is a simulator only code\n");
  exit(-1);
} 
