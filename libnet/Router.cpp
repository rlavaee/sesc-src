...
#ifdef DEBUG
#if 0
  MSG("@%lld router(%d)::launchMsg [%d:%d] to [%d:%d]", globalClock,
      myID,
      msg->getSrcRouterID(),
      msg->getSrcPortID(),
      msg->getDstRouterID(),
...
void Router::forwardMsg(Message *msg)
{
  ...
  case Message::PT_TO_PT:
    //NM commented, doesn't work(bug) wire = rTable->getWire(msg->getDstRouterID());
    wire = rTable->getWire((rTable->getWire(msg->getDstRouterID())->rID)); //NM*****//
    break;
  default:
  ...
  Time_t when = r2rPort[wire->port]->occupySlots(calcNumFlits(msg));
   ...
   //MSG("@%lld router::forwardMsg %d->%d, wire->dist=%d",globalClock,myID, wire->rID, wire->dist);
...
void Router::receiveMsg(Message *msg)
{
  ...
  Time_t when = r2lPort[portid]->occupySlots(calcNumFlits(msg));
   ...
   //MSG("dstport:%d srcport:%d router:%d @%lld",portid,msg->getSrcPortID(),myID, globalClock);

  msg->notifyMsgAbs(when, this);
  // NM commented and changed as above msg->notifyMsgAbs(when + calcNumFlits(msg), this);
}
...
void Router::notifyMsg(Message *msg)
{
  ...
  if (msg->getDelivery() != Message::PT_TO_PT) {
    ...
    //LOG("[%zu] received %s message from %zu @%lld", myID, 
    //	(msg->getDelivery() == Message::RCV_AND_PASS) ? "RCV_AND_PASS" : "RCV", 
    //	 msg->getSrcRouterID(),globalClock);
  ...
  //printf("notifyMsg : received message in %d @%lld\n",myID,globalClock);  //NM*****
